...
module.exports = {
    ...
    cooldown: 10,
    isSewa: true,
    isBlockCmd: checkDataId("blockcmd", "handhold", db.data), 
    isOwner: checkDataId("owner", "handhold", db.data), 
    isPremium: checkDataId("premium", "handhold", db.data),
    isLimit: checkDataId("limit", "handhold", db.data),
    isWait: true, 
    callback: async (sock, m, { prefix, command, setQuoted }) => {
        var { data } = await axios.get("https://waifu.pics/api/sfw/handhold")
