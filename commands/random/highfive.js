...
module.exports = {
    ...
    cooldown: 10,
    isSewa: true,
    isBlockCmd: checkDataId("blockcmd", "highfive", db.data), 
    isOwner: checkDataId("owner", "highfive", db.data), 
    isPremium: checkDataId("premium", "highfive", db.data),
    isLimit: checkDataId("limit", "highfive", db.data),
    isWait: true, 
    callback: async (sock, m, { prefix, command, setQuoted }) => {
        var { data } = await axios.get("https://waifu.pics/api/sfw/highfive")
