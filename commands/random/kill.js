...
module.exports = {
    ...
    cooldown: 10,
    isSewa: true,
    isBlockCmd: checkDataId("blockcmd", "kill", db.data), 
    isOwner: checkDataId("owner", "kill", db.data), 
    isPremium: checkDataId("premium", "kill", db.data),
    isLimit: checkDataId("limit", "kill", db.data),
    isWait: true, 
    callback: async (sock, m, { prefix, command, setQuoted }) => {
        var { data } = await axios.get("https://anime-api.hisoka17.repl.co/img/kill")
