...
module.exports = {
    ...
    cooldown: 10,
    isSewa: true,
    isBlockCmd: checkDataId("blockcmd", "poke", db.data), 
    isOwner: checkDataId("owner", "poke", db.data), 
    isPremium: checkDataId("premium", "poke", db.data),
    isLimit: checkDataId("limit", "poke", db.data),
    isWait: true, 
    callback: async (sock, m, { prefix, command, setQuoted }) => {
        var { data } = await axios.get("https://waifu.pics/api/sfw/poke")
