...
module.exports = {
    ...
    cooldown: 10,
    isSewa: true,
    isBlockCmd: checkDataId("blockcmd", "wave", db.data), 
    isOwner: checkDataId("owner", "wave", db.data), 
    isPremium: checkDataId("premium", "wave", db.data),
    isLimit: checkDataId("limit", "wave", db.data),
    isWait: true, 
    callback: async (sock, m, { prefix, command, setQuoted }) => {
        var { data } = await axios.get("https://waifu.pics/api/sfw/wave")
